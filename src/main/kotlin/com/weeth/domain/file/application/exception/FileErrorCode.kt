...
enum class FileErrorCode(
    ...
    FILE_NOT_FOUND(20600, HttpStatus.NOT_FOUND, "존재하지 않는 파일입니다."),

    @ExplainError("Presigned URL 생성 중 S3 연결 오류가 발생했을 때 발생합니다.")
    PRESIGNED_URL_GENERATION_FAILED(30600, HttpStatus.INTERNAL_SERVER_ERROR, "파일 업로드 URL 생성에 실패했습니다."),

    @ExplainError("허용되지 않은 Content-Type으로 파일 업로드를 시도했을 때 발생합니다.")
    UNSUPPORTED_CONTENT_TYPE(20601, HttpStatus.BAD_REQUEST, "지원하지 않는 파일 형식입니다."),

    @ExplainError("허용되지 않은 확장자로 파일 업로드를 시도했을 때 발생합니다.")
    UNSUPPORTED_FILE_EXTENSION(20602, HttpStatus.BAD_REQUEST, "지원하지 않는 파일 확장자입니다."),
}
