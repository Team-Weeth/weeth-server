    ...
    DescribeSpec({
        ...
        describe("create") {
            it("access/refresh token을 생성하고 저장한다") {
                every { jwtProvider.createAccessToken(1L, "a@weeth.com", "USER") } returns "access"
                every { jwtProvider.createRefreshToken(1L) } returns "refresh"

                val result = useCase.create(1L, "a@weeth.com", "USER")

                result shouldBe JwtDto("access", "refresh")
                verify(exactly = 1) { refreshTokenStore.save(1L, "refresh", "USER", "a@weeth.com") }
            }
        }
        ...
        describe("reIssueToken") {
            it("저장 토큰 검증 후 새 토큰을 재발급한다") {
                ...
                every { refreshTokenStore.getRole(10L) } returns "ADMIN"
                every { refreshTokenStore.getEmail(10L) } returns "admin@weeth.com"
                every { jwtProvider.createAccessToken(10L, "admin@weeth.com", "ADMIN") } returns "new-access"
                ...
                verify(exactly = 1) { refreshTokenStore.save(10L, "new-refresh", "ADMIN", "admin@weeth.com") }
            }
        }
    })
