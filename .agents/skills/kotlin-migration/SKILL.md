...
description: "Legacy Java → Kotlin migration skill. Use only when explicitly asked to migrate remaining Java files or external copied Java code. Follows Test-First methodology: write tests → migrate → refactor → verify with ktlint."
---
...
This repository's production migration is complete. Do not introduce new Java production code; use this skill only for explicit legacy cleanup.
