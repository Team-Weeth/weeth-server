---
name: context-update
description: Self-feedback skill that analyzes completed work and improves Codex context. Use when asked to "update context", "capture learnings", "improve context", or before compaction. Identifies reusable patterns and delegates to appropriate create skills.
---
...
Grep: pattern="{keyword}" path=".agents/"
```
...
2. Prefer existing project rules in `.agents/rules/` over ad-hoc new patterns.
3. If no rule exists, follow established skill workflows in `.agents/skills/*/SKILL.md`.
